    ...
    <Paper sx={{ p: 2, mb: 2 }}>
      ...
      <Typography variant="body1">
        ...
        {email.user_reassign && email.user_reassign !== email.label ? (
          <>
            <s style={{ color: 'red' }}>{topicMap[email.user_label_name]}</s> → {topicMap[email.user_reassign]}
          </>
        ) : (
  ...
  '& .MuiOutlinedInput-input': {
    ...
    width: '100%',
              ...
              >
                ...
                {Object.entries(allLabels).map(([key]) => (
  ...
  <Box display="flex" gap={2} height="100vh">
    ...
    <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
               ...
               <Box sx={{ flexShrink: 0, display: 'flex', flexDirection: 'column', py: 1 }}>
                        ...
                        <HorizontalRatioBar
                          title={`Rejection Rate ${rejectionstats?.mismatched_reassigned ?? 0}/${rejectionstats?.total_reassigned ?? 0}`}
                          value={
