...
def define_tables(metadata):
    ...
    ambigousset = Table(
        ambiguousset_tablename, metadata,
        Column("id", BigInteger, primary_key=True, autoincrement=True),  # 👈 new surrogate key         
        Column("row_hash", Text, ForeignKey("cleandatatext.row_hash"), nullable=False),
        Column("row_hash_model", Text, ForeignKey("tokenizeddata.row_hash_model"), nullable=False),  # can be row_hash or internal uid
        Column("confidence", Float, nullable=False),
        Column("lowest_difference", Float, nullable=False),
        Column("timestamp", Float, nullable=False),  # optional for tracking when label was added          
        Column("category_reassign_l1_source", ARRAY(Text), nullable=False),       
        Column("user_label_standard_frozen", ARRAY(Text), nullable=False),       

        Column("user_label_standard", ARRAY(Text), nullable=False),       
        Column("category_reassign_l1", ARRAY(Text), nullable=False),       
        Column("category_reassign_l1_distance", ARRAY(Text), nullable=False),       
        Column("category_reassign_l1_confidence", ARRAY(Text), nullable=False),               
        
        Column("created_at", DateTime(timezone=True), server_default=func.now(), nullable=False),
        Column("updated_at", DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False),
                       
        UniqueConstraint("row_hash_model", name="ambig_row_hash_model")  # ✅ Explicit constraint
    )    
    ...
    ambiguousset_tablename:ambigousset
    }